    ...
    def init_app(self, app):
        ...
        if isinstance(pika_params, connection.Parameters):
            ...
            if 'credentials' not in pika_params:
                pika_params['credentials'] = pika.PlainCredentials(pika_params['username'],
                                                                   pika_params['password'])
    ...
    def channel(self):
        ...
        if self.pool_recycle > -1:
            ...
            if self.__should_recycle_channel(ch):
                ...
                self.__DEBUG(
                    "Pika channel is too old, recycling channel %s and replacing it with %s" % (
                    old_channel, ch))
            else:
